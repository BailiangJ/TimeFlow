...
class FlowLoss(nn.Module):
    ...
    def forward(self,
        ...
        # dist: (B1HWD)
        # fg_mask: (B1HWD)
        spatial_dims = tuple(range(2, dist.ndim))
        if fg_mask is not None:
            if dist.shape != fg_mask.shape:
                output_size = dist.shape[2:]
                interp_mode = 'bilinear' if dist.ndim == 4 else 'trilinear'
                fg_mask = F.interpolate(fg_mask,
                                        align_corners=True,
                                        size=output_size,
                                        mode=interp_mode)
                if dist.shape[0] != fg_mask.shape[0]:
                    fg_mask = fg_mask.repeat(dist.shape[0], *([1] * (dist.ndim - 1)))
            assert dist.shape == fg_mask.shape

            if not val:
                loss = torch.sum(dist * fg_mask) / torch.sum(fg_mask)
            else:
                loss = (dist * fg_mask).sum(dim=spatial_dims) / fg_mask.sum(dim=spatial_dims)
        else:
            if not val:
                loss = torch.mean(dist)
            else:
                loss = dist.mean(dim=spatial_dims)
